...
from myexchange import get_price as start_processing
...
currency_dict = [['Российский рубль', 'рублей', 'рубль', 'Russian Ruble'],
                 ['Доллары сша', 'долларов', 'доллар', 'баксы', 'баксов', 'us dollar', 'US Dollar'],
                 ['Евро', 'евро', 'Euro'],
                 ]
...
def handle_values(message):
    ...
    for i in range(len(currency_list)):
        bot.send_message(message.chat.id, currency_list[i])
...
def handle_text(message):
    if message.text not in command_list:
        ...
        # base = input_list[0]    # валюта которую пересчитываем
        # quote = input_list[1]   # валюта в которую пересчитываем
        amount = input_list[2]  # необходимая сумма
        date_rate, rate_value, rp_list = start_processing(input_list)
